...
void BadTinker()
{
	...
	prob = random();
	if (prob <= 0.6) // Zilch
		sprint(self,PR_PRINT_HIGH,"Nothing happened.\n");
	else if (prob <= 0.8) // Zowie!
	{
		TF_T_Damage(self->building, world, world, self->building->health+1, 0, 0);
		sprint(self,PR_PRINT_HIGH,"The tesla blew up.\n");
	}
	else // ZAP!
	{
		self->building->tf_items = self->building->tf_items | PR_NIT_SECURITY_CAMERA; // Turns tesla against team.
		self->building->health = 100; // WK Make it easy to kill
		self->building->max_health = 100;
		sprint(self,PR_PRINT_HIGH,"You cut the wrong wire!\n");
	}
}

void DoTinker()
{
	float prob, thresh, enough;
	//local entity te;

	if (self->building->has_sentry > 0)
	{
		sprint(self,PR_PRINT_HIGH,"You have to fully upgrade before tinkering\n");
		return;
	}

	if (self->ammo_cells < 100) // not enough cells
		sprint(self,PR_PRINT_HIGH,"You need more cells to tinker on the tesla\n");
	else
	{
		...
		{
			if (!(self->building->tf_items & PR_NIT_SCANNER))
			{
				self->building->tf_items = (self->building->tf_items | PR_NIT_SCANNER);
				sprint(self,PR_PRINT_HIGH,"The tesla gets a free improved targeter!\n");
			}
			else
		...
		{
			if (!(self->building->tf_items & PR_NIT_AUTOID))
			{
				self->building->tf_items = (self->building->tf_items | PR_NIT_AUTOID);
				sprint(self,PR_PRINT_HIGH,"The tesla got a free spy detector!\n");
			}
			else
				Tesla_Add_Rand_Upgrade(self->building, self);
		}
